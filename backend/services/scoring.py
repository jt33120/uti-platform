...
def _reco(total: int) -> str:
    if total >= RECO_FORT_MIN:
        return "FORT"
    if total >= RECO_MOYEN_MIN:
        return "MOYEN"
    return "FAIBLE"


def score_consultant(features: dict, consultant: dict, ao: dict) -> dict:
    """
    ...
    comp_score = round(W_COMPETENCES * _clamp01(comp_ratio))
    ...
    if years is None:
        ...
        sen_ratio = _clamp01(max(years, 0) / SENIORITY_FULL_YEARS)
    sen_score = round(W_SENIORITE * sen_ratio)
    ...
    ctx_score = round(W_CONTEXTE * ctx_ratio)
    ...
    resume = (
        f"Score {total}/100 — compétences {comp_score}/{W_COMPETENCES}, "
        f"séniorité {sen_score}/{W_SENIORITE}, contexte {ctx_score}/{W_CONTEXTE}, "
